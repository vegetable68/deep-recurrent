...
void RNN::backward(const vector<string> &labels) {
  ...
  for (uint l=layers-1; l != (uint)(-1); l--) {
    ...
    for (uint i=T-1; i != (uint)(-1); i--) {
	  ...
	  outtmp += dhhf[l].col(i).norm();
    }
	dhhfout<<outtmp / T<<endl;
    ...
    for (uint i=0; i < T; i++) {
	  ...
	  outtmp += dhhb[l].col(i).norm();
    }
	dhhbout<<outtmp / T<<endl;
...
void RNN::update() {
  ...
  if (NORMALIZE)
	...
	for (int l = layers - 1; l >= 0; l --){
		vWWffout<<vWWff[l].squaredNorm()<<endl;
		vWWfbout<<vWWfb[l].squaredNorm()<<endl;
	}
	vWWffout<<endl;
	vWWfbout<<endl;
