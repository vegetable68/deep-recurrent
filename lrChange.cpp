...
class RNN {
	public:
		...
		Results train(vector<vector<string> > &sents, 
		...
		Matrix<double, 3, 2> testPerTag(vector<vector<string> > &sents, 
				vector<vector<string> > &labels, string btag, string itag, 
				uint btagpos, uint itagpos, 
				double& pProp, double& pBin, double& rProp, double& rBin, double& exP, double& exTr);

		Results testSequential(vector<vector<string> > &sents, 
				vector<vector<string> > &labels);
...
Results::Results(){
	for (int i = 0; i < 4; i ++) this->res[i] = Matrix<double, 3, 2>::Zero();
}

Results& Results::operator=(const Results& res1){
	for (int i = 0; i < 4; i ++) this->res[i] = res1.res[i];
	return *this;
}

Matrix<double, 3, 2> RNN::testPerTag(vector<vector<string> > &sents, 
		vector<vector<string> > &labels, string btag, string itag, 
		uint btagpos, uint itagpos, 
		double& pProp, double& pBin, double& rProp, double& rBin, double& exP, double& exTr) {
	...
	for (uint i=0; i<sents.size(); i++) { // per sentence
		...
		for (uint j=0; j<sents[i].size(); j++) {
			...
			if (maxi == btagpos)
				labelsPredicted.push_back(btag);
			else if (maxi == itagpos)
				labelsPredicted.push_back(itag);
			else
		...
		for (uint j=0; j<labels[i].size(); j++) { // per token in a sentence
			...
			if (t == btag) {
				...
				if (l1 != -1) tru.push_back(make_pair(l1,j));
				l1 = -1;
			} //else assert(false);

			if ((y == btag) || ((y == itag) && ((py == "") || (py == "O")))) {
				nExprPredicted++;
				if (l2 != -1)
				...
				if (l2 != -1) pred.push_back(make_pair(l2,j));
				l2 = -1;
			} else { 
				cout << y << endl;
				assert(false);
	...
	exP += nExprPredicted; exTr += nExprTrue;
	pProp += precNumerProp; pBin += precNumerBin;
	rProp += recallNumerProp; rBin += recallNumerBin;
...
Results RNN::testSequential(vector<vector<string> > &sents, 
		vector<vector<string> > &labels) {
	double pProp = 0, pBin = 0, rProp = 0, rBin = 0 , exP = 0, exTr = 0, f1Prop, f1Bin;	
	Results ret;
	for (uint i = 0; i < 3; i ++)
		ret.res[i] = testPerTag(sents, labels, TAG[i][0], TAG[i][1], i * 2 + 1, i * 2 + 2,
						pProp, pBin, rProp, rBin, exP, exTr);
	pProp = (exP == 0) ? 1 : pProp / exP;
	rProp = rProp / exTr;
	f1Prop = (2*pProp*rProp)/(pProp+rProp);
	pBin = (exP ==0) ? 1 : pBin/exP;
	rBin = rBin/exTr;
	f1Bin = (2*pBin*rBin)/(pBin+rBin);

	ret.res[3] << pProp, pBin,
		rProp, rBin,
		f1Prop, f1Bin;	
	return ret;
}
...
ostream& operator<<(ostream& out, const Results& res){
	for (int i = 0; i < 4; i ++){
		out<<outputLabel[i]<<":"<<endl;
		out<<"P, R, F1: "<<endl;
		out<<res.res[i]<<endl;
	}
	return out;
...
void RNN::update() {
	...
	for (uint l=layers-1; l<layers; l++) {
		if (l != layers - 1){
			...
			vWWfo[l] = 0.1*gWWfo[l] + mr*vWWfo[l];
			vWWbo[l] = 0.1*gWWbo[l] + mr*vWWbo[l];
		}
	}
...
int main(int argc, char **argv) {
	...
	for (DROP=0; DROP<0.1; DROP+=0.2) { // can use this loop for CV
		RNN brnn(25,25,25,TAGNO,LT);
		auto results = brnn.train(trainX, trainL, validX, validL, testX, testL);
		if (best < results) { // propF1 on val set
			best = results;
			bestDrop = DROP;
		}
		brnn.save("Alloutput/model.txt");
	}
	cout << "Best: " << endl;
