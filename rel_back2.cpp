...
class Classifier{
	...
	private:
		...
		int nhf;
		double beta0;
		MatrixXd gbetaArgf, gbetaDSEf, gbetaArgb, gbetaDSEb;
...
void Classifier::diag(){
	...
	cout<<"betaARGf:"; cout<<betaArgf<<endl;
	cout<<"betaDSEb:"; cout<<betaDSEb<<endl;
	cout<<"betaARGb:"; cout<<betaArgb<<endl;
...
double Classifier::h(vector<MatrixXd> dseSpanf, vector<MatrixXd> argSpanf, vector<MatrixXd> dseSpanb, vector<MatrixXd> argSpanb){
	...
	//cerr<<"h starts"<<endl;
	y << 0.;
	MatrixXd dsef = MatrixXd(2 * nhf, 1);
	assert(dseSpanf.size() > 0);
	dsef << dseSpanf[0], dseSpanf[dseSpanf.size() - 1];
	MatrixXd dseb = MatrixXd(2 * nhf, 1);
	assert(dseSpanb.size() > 0);
	dseb << dseSpanb[0], dseSpanb[dseSpanb.size() - 1];
	MatrixXd argf = MatrixXd(2 * nhf, 1);
	assert(argSpanf.size() > 0);
	argf << argSpanf[0], argSpanf[argSpanf.size() - 1];
	MatrixXd argb = MatrixXd(2 * nhf, 1);
	assert(argSpanb.size() > 0);
	argb << argSpanb[0], argSpanb[argSpanb.size() - 1];

	y = y + betaDSEf * dsef + betaDSEb * dseb + betaArgf * argf + betaArgb * argb;
	y(0, 0) = y(0, 0) + beta0;
	//cerr<<"h() "<<endl;
...
double Classifier::trainh(AnsPair cur){
	Matrix<double, 1, 1> y;
	y << 0.;
	assert(betaDSEf.cols() == cur.dseSpanf.rows());
	assert(betaDSEb.cols() == cur.dseSpanb.rows());
	assert(betaArgf.cols() == cur.argSpanf.rows());
	assert(betaArgb.cols() == cur.argSpanb.rows());
...
MatrixXd Classifier::getSum(vector<MatrixXd> span){
	assert(!span.empty());
	MatrixXd res = MatrixXd(2 * nhf, 1);
	assert(span[0].rows() == nhf);
	res << span[0], span[span.size() - 1];
	return res / span.size();
}
...
void Classifier::train(RNN argnn, RNN dsenn,
	...
	for (int iter = 0; iter < MAX_ITER; iter ++){
		 ...
		 gbetaDSEf = RowVectorXd::Zero(2 * nhf);
		 gbetaArgf = RowVectorXd::Zero(2 * nhf);
		 gbetaDSEb = RowVectorXd::Zero(2 * nhf);
		 gbetaArgb = RowVectorXd::Zero(2 * nhf);
		 gbeta0 = 0;
		 xx = 0;
		 validPairs = 0;
		 tmp = 0;
...
void Classifier::backprop(RNN argnn, RNN dsenn, vector<vector<string> > label,
	...
	//cerr<<"backprop "<<argPair.size()<<endl;
				for (int k = 0; k < argPair.size(); k ++){
					...
					//cerr<<j<<endl;
					assert(betaArgf.cols() == 2 * nhf);
					//cerr<<nhf<<endl;
					argdhhf.col(argPair[k].first).noalias() += double(predict - ans) * betaArgf.block(0, 0, 1, nhf).transpose();
					//cerr<<"argdhhf-upper "<<argPair[k].second<<" "<<argdhhf.cols()<<endl;
					argdhhf.col(argPair[k].second - 1).noalias() += double(predict - ans) * betaArgf.block(0, nhf, 1, nhf).transpose();
					//cerr<<"argdhhf-lower"<<endl;

					assert(betaArgb.cols() == 2 * nhf);
					argdhhb.col(argPair[k].first).noalias() += double(predict - ans) * betaArgb.block(0, 0, 1, nhf).transpose();
					//cerr<<"argdhhb-upper"<<endl;
					argdhhb.col(argPair[k].second - 1).noalias() += double(predict - ans) * betaArgb.block(0, nhf, 1, nhf).transpose();
					//cerr<<"argdhhb-lower"<<endl;

					assert(betaDSEf.cols() == 2 * nhf);
					dsedhhf.col(start).noalias() += double(predict - ans) * betaDSEf.block(0, 0, 1, nhf).transpose();
					//cerr<<"dsedhhf-upper"<<endl;
					dsedhhf.col(j - 1).noalias() += double(predict - ans) * betaDSEf.block(0, nhf, 1, nhf).transpose();
					//cerr<<"dsedhhf-lower"<<endl;

					assert(betaDSEb.cols() == 2 * nhf);
					dsedhhb.col(start).noalias() += double(predict - ans) * betaDSEb.block(0, 0, 1, nhf).transpose();
					//cerr<<"dsedhhb-upper"<<endl;
					dsedhhb.col(j - 1).noalias() += double(predict - ans) * betaDSEb.block(0, nhf, 1, nhf).transpose();
					//cerr<<"dsedhhb-lower"<<endl;
				}
			}
			curDsef.clear(); curDseb.clear();
			if (maxi == 1){
...
RowVector3d Classifier::test(RNN argnn, RNN dsenn,
	...
	cout<<trupredict<<","<<tru_pos<<","<<tt<<endl;
...
Classifier::Classifier(uint _nhf, double _alpha){
	...
	//cerr<<beta0<<endl;
	betaArgf = RowVectorXd(2 * nhf).unaryExpr(ptr_fun(urand));
	betaDSEf = RowVectorXd(2 * nhf).unaryExpr(ptr_fun(urand));
	betaArgb = RowVectorXd(2 * nhf).unaryExpr(ptr_fun(urand));
	betaDSEb = RowVectorXd(2 * nhf).unaryExpr(ptr_fun(urand));
