...
class Classifier{
	public:
		Classifier(uint _nhf, string fname);
...
Classifier agent_dse(25, "agent_dse"), target_dse(25, "target_dse");
...
void Classifier::output(RNN argnn, RNN dsenn,
	...
	cout<<fname<<":"<<double(pos) / total<<endl;
}
		...
		if (maxi == 0 || maxi == 1){
			if (!curDsef.empty()){
				...
				for (int k = 0; k < argPair.size(); k ++){
					...
					argdhhf.col(argPair[k].second - 1).noalias() += double(1 - predict) * beta.block(3 * nhf, 0, nhf, 1).transpose();
					argdhhb.col(argPair[k].first).noalias() += double(1 - predict) * beta.block(2 * nhf, 0, nhf, 1).transpose();
					dsedhhf.col(j - 1).noalias() += double(1 - predict) * beta.block(nhf, 0, nhf, 1).transpose();
					dsedhhb.col(start).noalias() += double(1 - predict) * beta.block(0, 0, nhf, 1).transpose();
				}
			}
			curDsef.clear(); curDseb.clear();
			if (maxi == 1){
...
Classifier::Classifier(uint _nhf, string _fname){
	nhf = _nhf;
	fname = _fname;
